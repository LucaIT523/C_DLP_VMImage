...
void	MyZipData(std::wstring	p_DataPath, std::wstring	p_ZipFilePath)
{
	...
	std::wstring	directory = p_DataPath;
	...
	newZip.write(ConvertWStringToString(directory), "vmimge/", folderAttr);

	//. Folder
	for (const auto& folder : folders) {
		std::wstring	w_subFolderName = L"vmimge/";
	...
	for (const auto& file : files) {
		std::wstring	w_subFileName = L"vmimge/";
	...
	newZip.save(ConvertWStringToString(p_ZipFilePath));
}
